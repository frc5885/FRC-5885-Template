...
// /**
//  * A utility class for mapping values using various smoothing methods. The class uses a TreeMap
// to
//  * store the mapped values and provides methods to add new values to the map and retrieve the
// mapped
//  * value for a given input using the set smoothing method. The smoothing method can be set using
// the
//  * SMOOTHING_METHOD enum, which includes various easing functions. The class also includes
// methods
...
//   // Sets the easing function to be used when mapping the values. Higher the power the "stepper"
// the
...
//   /**
//    * This class provides methods for mapping input values to output values using different
// smoothing
...
//    */
//   public void put(double[] x, double[] y) {
//     if (x.length != y.length) throw new IllegalArgumentException("Arrays must be the same
// length");
...
//   /**
//    * Maps a value to a corresponding output value using a set of input-output pairs and a
// smoothing
//    * function.
//    *
...
//       case EASE_QUAD:
//         return (double) (p < 0.5 ? 2.0 * p * p : 1.0 - Math.pow(-2.0 * p + 2.0, 2.0) / 2.0) *
// y_dif
...
//             + lower_entry.getValue();
//       case EASE_QUART:
//         return (double) (p < 0.5 ? 8.0 * p * p * p * p : 1.0 - Math.pow(-2.0 * p + 2.0, 4.0) /
// 2.0)
//                 * y_dif
//             + lower_entry.getValue();
//       case EASE_EXPO:
